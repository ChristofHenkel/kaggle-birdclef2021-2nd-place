...
class Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        all_logits = []
        for parts in [120, 60, 30]:
            n_samples = x_orig.shape[2]
            x = x_orig.permute(0, 2, 1, 3)
            x = x[:, : (n_samples // parts) * parts, :, :].reshape(
                parts, n_samples // parts, x.shape[2], x.shape[3]
            )
            x = x.permute(0, 2, 1, 3)
            ...
            logits = logits.repeat_interleave(120 // parts, dim=0)
